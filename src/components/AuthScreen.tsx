...
import { 
  ...
  Lock, AlertCircle, Sparkles, KeyRound, Smartphone,
  Radio, Send, ShieldCheck, Mail, Eye, EyeOff
} from 'lucide-react';
...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin }) => {
  // Two clean primary modes: Sign Up (Sajili) or Log In (Ingia)
  const [activeTab, setActiveTab] = useState<'signup' | 'login'>('login');

  // Sign Up Role Selector: Super User vs Primary User
  const [selectedRole, setSelectedRole] = useState<UserRole>('primary_user');

  // Sign Up Fields: Full Name, Email, Phone Number, Password, Designation
  ...
  // OTP State (Alternative Phone Login)
  ...
  // Saved Registered Profiles (purging legacy mock users)
  ...
  /**
   * Acquire phone coordinates via GPS and link to DIRA map
   */
  ...
  /**
   * Handle user self-registration as Super User or Primary User
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (!cleanPhone || cleanPhone.length < 9) {
      setAuthError('Please enter a valid phone number for dispatch and alerts.');
      return;
    }
    if (!cleanPass) {
      setAuthError('Please create a password for your account (minimum 4 characters).');
      return;
    }
    ...
    if (!cleanDesignation) {
      setAuthError('Please enter your designation (e.g. Village Elder, Area Chief, OCS, Donkey Owner).');
      return;
    }
    ...
    // Pick phone location and log in
    capturePhoneLocationAndFinish(newProfile);
  };

  /**
   * Request OTP code for phone login (alternative fallback)
   */
  const handleRequestOtp = (phoneOrUser: string | UserProfile) => {
    ...
    if (typeof phoneOrUser === 'string') {
      ...
      if (!targetPhone) {
        setAuthError('Please enter your phone number or "admin" to receive login SMS.');
        return;
      }
      if (targetPhone.toLowerCase() === 'admin') {
        ...
        if (found) {
          ...
          setAuthError('No account found with this phone number. Please click "Sign Up (Sajili)" above to register as Super User or Primary User.');
          return;
        }
      }
    } else {
  ...
  const handleVerifyLogin = async (e?: React.FormEvent) => {
    ...
    // If user is using SMS OTP fallback
    if (loginMethod === 'otp' && isOtpSent && otpTargetUser) {
      ...
      if (entered === expected || entered === defaultPin.trim() || (otpTargetUser.role === 'super_admin' && entered.toLowerCase() === 'admin')) {
        ...
        setAuthError(`Invalid code. Enter SMS code [${expected}], default PIN 1234, or 'admin'.`);
      }
      return;
    }
    ...
    try {
      ...
      if (authResult.success && authResult.user) {
        ...
        setAuthError(authResult.error || 'Authentication failed. Please check your credentials.');
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] w-full max-w-full overflow-x-hidden bg-zinc-950 flex flex-col justify-center items-center p-3 sm:p-4 text-zinc-100 selection:bg-red-800 selection:text-white">
      
      {/* SMS TOAST ON TOP */}
      {activeSms && (
        <div className="fixed top-3 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-md bg-zinc-900 border-2 border-red-600 rounded-2xl shadow-2xl p-3 animate-in slide-in-from-top duration-200">
      ...
      <div className="w-full max-w-md text-center mb-4 space-y-1">
        <div className="flex justify-center mb-2">
          <DiraLogo size="lg" showText={false} />
        </div>
        <h1 className="text-2xl font-black font-display tracking-tight text-white flex items-center justify-center gap-2">
          DIRA • KAA RADA!
        </h1>
        <p className="text-xs font-medium text-zinc-400">
          Caritas Kitui Donkey Welfare & Crime Prevention Network
        </p>
      </div>

      {/* Main Authentication Card */}
      <div className="w-full max-w-md bg-white text-zinc-900 rounded-3xl shadow-2xl border border-zinc-200 overflow-hidden flex flex-col">
        
        {/* GPS Indicator Banner */}
        {geoLocating && (
          <div className="bg-blue-600 text-white px-3 py-1.5 text-xs font-bold flex items-center justify-center gap-2 animate-pulse">
            <Radio className="w-3.5 h-3.5 animate-spin" />
            <span>Detecting phone GPS coordinates & linking to DIRA Map...</span>
          </div>
        )}

        {/* Top Tab Switch: Sign Up vs Log In */}
        <div className="p-3 bg-zinc-100 border-b border-zinc-200">
          <div className="grid grid-cols-2 gap-1.5 p-1 bg-zinc-200/80 rounded-2xl text-xs font-bold">
            <button
              onClick={() => {
                setActiveTab('signup');
                setAuthError(null);
        ...
        <div className="p-4 sm:p-5 overflow-y-auto max-h-[75vh] space-y-4">
          ...
          {/* TAB 1: SIGN UP (STREAMLINED: ROLE DROPDOWN + NAME, PHONE NUMBER, DESIGNATION + LOG IN ICON AT BOTTOM) */}
          {activeTab === 'signup' && (
          ...
          {/* TAB 2: LOG IN (PHONE NUMBER + PIN / SMS CODE) */}
          {activeTab === 'login' && (
                    ...
                    {savedProfiles.map((profile) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-[#991B1B] hover:bg-[#7F1D1D] text-white text-[10px] font-black rounded-xl border border-red-800 shrink-0 shadow-xs cursor-pointer"
                        >
                          Log In →
                        </button>
                      </div>
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="truncate">
                    ...
                    <div className="text-[10px] text-purple-700 truncate">
                      Manage Firebase users, add/delete accounts & permissions
                    </div>
                  </div>
                </div>
                <button
                  type="button"
                  id="btn-super-admin-direct-login"
                  onClick={() => {
