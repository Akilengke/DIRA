...
import { 
  ...
  ChevronRight, PhoneCall, ShieldAlert, Sparkles, AlertTriangle, Eye, CheckCheck, LogIn
} from 'lucide-react';
...
export const MyReportsView: React.FC<MyReportsViewProps> = ({
  ...
  // Filter cases to strictly show only those reported by the logged-in user
  ...
  return (
    <div className="space-y-3.5 pb-16">
      {/* Header Banner in Brick Red */}
      <div className="bg-gradient-to-r from-[#991B1B] via-[#7F1D1D] to-[#991B1B] text-white p-4 sm:p-5 rounded-3xl shadow-sm border border-red-800 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-red-100 mt-0.5">
            Monitor real-time investigation updates from Caritas Kitui field officers.
          </p>
        </div>
        <button
          id="my-reports-new-case-btn"
          onClick={onOpenReportModal}
          className="bg-white hover:bg-zinc-100 text-[#991B1B] px-3.5 py-2.5 rounded-2xl text-xs font-bold flex items-center gap-1.5 shadow-md active:scale-95 transition-all shrink-0"
        >
          ...
          userCases.map((c) => {
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px] bg-zinc-50 p-2.5 rounded-xl border border-zinc-100">
                  <div className="flex items-center gap-1.5 text-zinc-600 truncate">
                    <MapPin className="w-3.5 h-3.5 text-[#991B1B] shrink-0" />
                    <span className="truncate">{c.location.subCounty}, {c.location.village}</span>
                  </div>
